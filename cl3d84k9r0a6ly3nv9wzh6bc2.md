...
While it has recently garnered much attention DSNs are not a new thing. The first generation of p2p DSNs like Napster, Torrents, etc. was often used for sharing music and video files.  Peers maintained copies of fragments of files on their computers. This fragment is then shared with other node(s) in the network, upon request. Peers could decide to stay online or not, with Torrents, the more a peer provided to the network, the more it enjoyed the download process. 
